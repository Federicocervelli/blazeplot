...
- Branch preview index: <https://blazeplot.cervelli.dev/previews.html>
...
bun run docs:readme        # regenerate docs/api-reference.md from dist/
bun run ci
bun run pages:build
bun pm pack --dry-run
```
...
- `bun run docs:readme`: rebuilds the package and regenerates `docs/api-reference.md` from `dist/`.
...
The API reference bundle-size summary is generated by `bun run docs:readme` from the built `dist/` files. It uses the same budgets as `bun run test:bundle-size`, reports the current size for each budgeted chunk, and shows how much budget remains for each chunk.
