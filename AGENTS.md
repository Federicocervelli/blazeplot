...
- `MinMaxPyramid` updates incrementally for tail appends and falls back to full rebuild on explicit rebuild/clear. `SeriesStore` detects ring shifts at fixed capacity and avoids per-frame full pyramid rebuilds; dense min/max extraction then uses a raw min/max scan fallback (`RingBuffer.minMaxY` fast path). Dense non-wrapped extraction uses `rangeMinMax()` over pyramid buckets.
