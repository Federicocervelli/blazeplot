...
- GitHub Pages deploys on pushes to `main` and `development`. The stable site is served at `https://blazeplot.cervelli.dev/`; the in-progress `development` site is served at `https://blazeplot.cervelli.dev/development/`; integrated Lit previews live under `#previews`; `previews.html` links the branch sites/previews.
